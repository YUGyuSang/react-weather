...
function App() {
  ...
  const getWeatherByCity= async()=>{
    setLoading(true)
    let url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=1aa3253a785fd768b157ee07b30643f0&units=metric`
    let response = await fetch(url)
    let data = await response.json()
    console.log("getweatherbucity data?", data);
    setWeather(data);
    setLoading(false)
  }
